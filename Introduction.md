...
$$
一般地，高斯分布的概率密度函数PDF写为（其中$x \in \mathbb{R}^{p\times 1}$）：
...
$$
\begin{aligned}
\log p(X|\theta)&=\sum\limits_{i=1}^{N}\log p(x_{i}|\theta)=\sum\limits _{i=1}^{N}\log\frac{1}{\sqrt{2\pi}\sigma}\exp(-\frac{(x_{i}-\mu)^{2}}{2\sigma^{2}})\\
&=\sum_{i=1}^{N}[\log(\frac{1}{\sqrt{2\pi}})+\log\frac{1}{\sigma}-\frac{(x_{i}-\mu)^{2}}{2\sigma^{2}}]
\end{aligned}
$$
首先对 $\mu$ 的极值可以得到 ： 
$$
...
多维高斯分布表达式为：
$$
x\sim N(\mu,\Sigma)=\frac{1}{(2\pi)^{p/2}|\Sigma|^{1/2}}e^{-\frac{1}{2}(x-\mu)^{T}\Sigma^{-1}(x-\mu)}
$$
其中 $x,\mu\in\mathbb{R}^{p},\Sigma\in\mathbb{R}^{p\times p}$ ，$\Sigma$ 为协方差矩阵，一般而言也是半正定矩阵。这里我们只考虑正定矩阵。首先我们处理指数上的数字，指数上的数字可以记为 $x$ 和 $\mu$ 之间的马氏距离，当$\Sigma =\mathbb{I}$时,马氏距离与欧式距离相等。对于对称的协方差矩阵可进行特征值分解，$\Sigma=U\Lambda U^{T}=(u_{1},u_{2},\cdots,u_{p})diag(\mathbf{\lambda})(u_{1},u_{2},\cdots,u_{p})^{T}=\sum\limits _{i=1}^{p}u_{i}\lambda_{i}u_{i}^{T}$ ，于是：
$$
...
我们令$y_i = (x-\mu)^T u_i$,注意到 $y_{i}$ 是 $x-\mu$ 在特征向量 $u_{i}$ 上的投影长度，因此上式子就是 $\Delta$ 取不同值时的同心椭圆。

二维的高斯分布可以形象的表现出来。

![](D:\blog\机器学习\figure\马氏距离.png)

### 高斯分布的局限性

下面我们看多维高斯模型在实际应用时的两个问题

1.  参数 $\Sigma,\mu$ 的自由度为 $O(p^{2})$ 对于维度很高的数据其自由度太高，学习或者估算的难度较大。解决方案：高自由度的来源是 $\Sigma$ 有 $\frac{p(p+1)}{2}$ 个自由参数，可以假设其是对角矩阵（超椭球的轴线与基相同），甚至在各向同性假设中假设其对角线上的元素都相同（超球体）。前一种的算法有 Factor Analysis，后一种有概率 PCA(p-PCA) 。
...
>   \end{align}
>   $$
>   注意到协方差矩阵的对称性，所以 $p(z)\sim\mathcal{N}(\begin{pmatrix}\mu\\A\mu+b\end{pmatrix},\begin{pmatrix}\Lambda^{-1}&\Lambda^{-1}A^T\\A\Lambda^{-1}&L^{-1}+A\Lambda^{-1}A^T\end{pmatrix})$。根据之前的公式，我们可以得到：
>   $$
...
有关$Var(z)=\begin{pmatrix}\Sigma_{xx}&\Sigma_{xy}\\\Sigma_{yx}&\Sigma_{yy}\end{pmatrix}$的证明: